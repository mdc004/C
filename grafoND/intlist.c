...
/* Inserisce un nodo in testa alla lista concatenata, impostando il suo campo dati a it. 
    Il nuovo nodo viene allocato dinamicamente, e i suoi datti letti dall'input*/
intlist* list_insert2(intlist* head, item it){
	intlist * tmp = malloc(sizeof(intlist));
	if(tmp){
		copy_item(&tmp->dati, it);
		tmp -> next = head;
		head = tmp;
	}else	printf("\t list_insert: Errore allocazione nodo\n");		
	return head; 
...
intlist* list_search(intlist* head, key k){
	...
	}else printf("\tAttenzione: lista vuota\n");	
	return tmp;
}
